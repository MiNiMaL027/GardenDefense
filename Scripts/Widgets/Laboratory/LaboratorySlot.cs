...
public partial class LaboratorySlot : Panel
{
    ...
    private void BuyButton_Pressed()
    {
        ...
        if (playerController.GardenInventory.CountOfItem(HarvestId) < HarvestCount)
            return;
  
        playerController.Gold -= GoldPrice;
		playerController.GardenInventory.RemoveItem(HarvestId, HarvestCount);
		playerController.GardenInventory.AddItem(BattlePlantId);
